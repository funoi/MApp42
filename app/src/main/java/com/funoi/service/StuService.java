...
public class StuService {
    ...
    public void addStu(Student s) {
        ...
        String hobby = s.getHobby().toString().substring(1, s.getHobby().toString().length() - 1);
        stu.put("hobby", hobby);
    ...
    public void delStu(Integer id) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();  // 写入的方式打开数据库
        db.delete("student", "id=?", new String[]{String.valueOf(id)});  // 调用 db 操作对象将对应 id 的记录删除
    }
    ...
    public Student findStu(Integer id_) {
        ...
        Cursor cursor = db.query("student", null,
                "id=?", new String[]{String.valueOf(id_)}, null, null, null);
        ...
        if (cursor.moveToFirst()) {
            ...
            String[] str = cursor.getString(cursor.getColumnIndexOrThrow("hobby")).split(", ");
